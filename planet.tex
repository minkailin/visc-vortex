...
\begin{table*}
  ...
  \caption{Summary of disc-planet simulations. These runs employ the
    ...
    \begin{tabular}{llllrl}
      ...
      Pb0     & 1.0  & 1          & 0      & 9.2  & single vortex by $t=130P_0$, no
      vortex after $t=160P_0$   \\ %doing HR
      Pb0.5   & 1.0  & 10         & $0.5H$ & 7.3  & similar to Pb0     \\ 
      Pb1     & 1.0  & 10         & $H$    & 5.7  &    single vortex
      by $t=130P_0$, disappears after $t=140P_0$   \\ %doing HR

      Pc0.5   & 1.0  & 100          & $0.5H$ &  &     \\ %optional
      Pc1     & 1.0  & 100          & $H$    &  &     \\ %optional  
      \hline
  \end{tabular}
\end{table*}
...
We also performed several simulations with a floor
viscosity of $\hat{\nu}_0=10^{-6}$ and a viscosity boost $A_\nu=10$
and $A_\nu=100$ in the viscous layer.  


\subsubsection{Strictly isothermal discs} 

Locally isothermal discs are vulnerable to the vertical shear
instability because $\partial_z\Omega_i\neq 0$
\citep{nelson12}. This instability involves small radial
wavelengths ($\ll H$), and so \citeauthor{nelson12} employed a radial
resolution $\gtrsim 60$ cells per $H$ to simulate it. Thus the 
the numerical resolutions adopted for our simulations are unlikely to
capture this instability. Nevertheless, we repeated 
simulations P0, P1 and Q1 with a strictly isothermal equation of state
($q=0$) in order to eliminate the potential for vertical shear
instability. These are summarised in Table \ref{planet_sims_iso}. 

\begin{table}
  \centering
  \caption{Disc-planet simulations with a strictly
    isothermal equation of state ($q=0$). These runs employ the
    `wedge' viscosity model described by
    Eq. \ref{planet_visc_profile}. The thickness of the viscous layer
...
\begin{figure}
  \centering
   \includegraphics[scale=.27,clip=true,trim=0cm 0.cm 0cm
     0.9cm]{figures/jup0_3h_iso_pdisk_vorten_015}\includegraphics[scale=.27,clip=true,clip=true,trim=2.3cm
     0.cm 0cm
     0.9cm]{figures/jup1_3h_iso_amp10_pdisk_vorten_015}
   \includegraphics[scale=.27,clip=true,clip=true,trim=2.3cm
     0.cm 0cm
     0.9cm]{figures/jup1_3h_iso_pdisk_vorten_015}
   \caption{Isothermal disc-planet simulations. The perturbed inverse
     PV, $\eta_z(t=0)/\eta_z - 1$, is shown for case Iso0 (left, no
     viscous layer), case Iso1 (middle, viscous layer of $H_0$ with
     $\hat{\nu}\sim10^{-5}$) and case Iso2 (right, viscous layer of
     $H_0$ with $\hat{\nu}\sim10^{-4}$). Snapshots are taken at
     $t=150P_0$. 
     \label{iso_vorten}}
\end{figure}
